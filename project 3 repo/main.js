...
;(function () {
    ...
    //can you change radius of circles?
    let radiusScale = d3.scaleSqrt().domain(['first','second','third', 'fourth', 'fifth']).range([1, 2, 3, 4, 5])
...
//    let colorScale = d3.scaleOrdinal().domain(['first','second','third', 'fourth']).range(['blue'])
    ...
    datapoints.forEach(d => {
        ...
        d.fifthCircleCoords = getCircleXY(15)
    
        d.color = colorScale(d.round)
        d.radius = 2.2
        // d.radius = radiusScale(d.round)
        // reduce opacity??
    })
